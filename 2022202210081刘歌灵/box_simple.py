...
'''
算法说明:
最简单的贪心算法,从(0,0,0)位置开始放第一个物品,将当前物品放下后下一个物品可放置的顶点坐标存入pos_list队列中。
当要放入第二个物品时,遍历队列寻找可以放下的位置,并可以尝试将长边和宽边换方向。
一旦放下后就更新pos_list队列,将第二个物品占用的pos删掉,将新产生的3个pos点加入队列。
直到车厢的空间不足以放下下一个物品,或者所有的pos都不配合剩下的物品。
考虑到箱子是有同等规模的多个,所以在选择放入位置的时候,优先把底层填满
也就是对pos_list做一个排序,靠底部的优先
'''
'''
算法效果:
平均能够达到80.09%
如果加一个可以按高边放的话,可以达到80.94%
'''
...
def main():
    ...
    rate_all = []
    for c,config in enumerate(box_configs):
        truck_l = 587
        truck_w = 233
        truck_h = 220
        ...
        rate_all.append(1-truck.volumn/(truck_l*truck_w*truck_h))
        make_pic(fit_in_boxs) #画出最终车厢放置的示意图
        print("第{}个config,放入物品数:{}".format(c,fit_in_num))  
        print("车厢填充率{}".format(1-truck.volumn/(truck_l*truck_w*truck_h)))
