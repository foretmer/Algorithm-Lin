...
class Item:
    def __init__(self,name,length,width,height,weight):
        ...
        from Auxiliary_Method import START_POSITION
        self.position = START_POSITION  # initial position: (0, 0, 0)
        from Auxiliary_Method import DEFAULT_NUMBER_OF_DECIMALS
        self.number_of_decimals = DEFAULT_NUMBER_OF_DECIMALS

    def format_numbers(self,number_of_decimals):
        from Auxiliary_Method import set_to_decimal
    ...
    def get_volume(self):
        from Auxiliary_Method import set_to_decimal
        return set_to_decimal(self.length * self.height * self.width, self.number_of_decimals)
